...
import numpy as np
import cv2
from arenaclient.sc2 import game_state, game_info, bot_ai, position, cache, game_data, unit
from arenaclient.sc2.ids.unit_typeid import UnitTypeId
import random as rd
...
class Minimap:
    def __init__(
            ...
            custom_points={},
            player_name=None
            # {Point2([100, 50]): (200, 50, 50),(Point2([101, 120]),Point2([101, 135]),Point2([101, 150])): (200, 250, 250)}
    ):
    ...
    def add_vision_blockers(self, map_data):
        vb = map_data.copy()
        for p in self._game_info.vision_blockers:
    ...
    def add_creep(self, map_data):
        ...
        for (y, x), v in np.ndenumerate(self._state.creep.data_numpy):
            if v:
                cv2.rectangle(
                    map_data,
    ...
    def add_psi(self, map_data):
        ...
        for psi_provider in self._state.own_units(UnitTypeId.PYLON):
    ...
    def add_xelnaga(self, map_data):
        for unit in self._state.watchtowers:
            cv2.circle(
                map_data,
                (
    ...
    def add_destructables(self, map_data):
        ...
        for unit in self._state.destructables:
            cv2.circle(
                map_data,
                (
                    int(unit.position[0] * self.map_scale),
    ...
    def add_allies(self, map_data):
        for unit in self._state.own_units:
    ...
    def add_enemies(self, map_data):
        for unit in self._state.enemy_units:
            if unit.is_structure:
                cv2.rectangle(
                    map_data,
                    (
    ...
    def add_visibility(self, map_data):
        ...
        # gets the min and max height of the map for a better contrast
        v_min = np.amin(self._state.visibility.data_numpy)
        v_max = np.amax(self._state.visibility.data_numpy)
        multiplier = 255 / (v_max - v_min)

        for (y, x), v in np.ndenumerate(self._state.visibility.data_numpy):
    ...
    def add_custom_points(self, map_data):
        for pointlist, color in self.custom_points.items():
            if isinstance(pointlist, position.Point2):
                cv2.circle(
                    map_data,
                    (
    ...
    def load_game_info(self, data):
        ...
        # if not self.ramps_loaded:
        #     self._map_ramps, self._vision_blockers = self._game_info._find_ramps_and_vision_blockers()
        #     self.ramps_loaded = True
        # self._game_info.map_ramps, self._game_info.vision_blockers = self._map_ramps, self._vision_blockers
